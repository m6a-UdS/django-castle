...
class Castle(object):
    ...
    def log_login_success(self, user, request, ip_header=None):
        ip_header = ip_header if ip_header else self.default_ip_header
        headers = self.get_headers_from_request(request, ip_header=ip_header)
    ...
    def log_login_fail(self, credentials, ip_header=None):
        ip_header = ip_header if ip_header else self.default_ip_header
    ...
    def log_logout_success(self, user, request, ip_header=None):
        ip_header = ip_header if ip_header else self.default_ip_header
        headers = self.get_headers_from_request(request, ip_header=ip_header)
        user_id = castle_userid(user) if user else ""
        self.make_request("events", data={"name": "$logout.succeeded", "user_id": user_id}, headers=headers)

    def log_user_registration_success(self, request, ip_header=None):
        ip_header = ip_header if ip_header else self.default_ip_header
        headers = self.get_headers_from_request(request, source=default_source, ip_header=ip_header)
