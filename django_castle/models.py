...
def catch_login_signal(sender, request, user=None, enable_logging=False, **kwargs):
    castle = Castle()
    if not request.user:
        request.user = user
...
def catch_logout_signal(sender, request, user=None, enable_logging=False, **kwargs):
    castle = Castle()
    if not request.user:
        request.user = user
